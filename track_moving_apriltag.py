"""Tracks an AprilTag mounted on a LEGO Education Double Motor using a webcam,
and drives the Double Motor to keep the tag centered horizontally in frame.

The robot is assumed to sit side-on to the camera, with its two wheels/tracks
oriented so that driving both at the same speed and direction translates it
left/right across the frame (rather than turning it). A PID controller turns
the tag's horizontal pixel error (distance from the frame's center column)
into a single translation speed applied equally to both wheels, clamped to
a configurable min/max speed range.
...
mismatch mentioned separately - that's a planned follow-up (using the Double
Motor's built-in gyroscope to trim for heading drift).
"""
...
# to a positive pixel error (tag right of center). Flip to -1 if the robot
# drives the wrong way during testing.
DIRECTION_SIGN = -1
...
# forcing a stop, in seconds.
LOST_TAG_TIMEOUT_S = 0.5
...
def main():
    ...
    try:
        ...
        while cap.isOpened():
            ...
            # mounting relative to the camera.
